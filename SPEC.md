...
Оновлено: 2026-09-11. Формат — за розділом 5 AGENTS.md.
...
**Контекст:** одна канонічна версія `versions/en-US-landing/` (стратегія «довести до результату → розмножити»). **Платформене рішення (2026-09-09):** продакшн переїхав на **Cloudflare Pages** під бойовим доменом **https://biosunlocktool.com/** — монорепо `vaoferi/biosunlocktool` з i18n-версіями та короткими market-subdomains: `us`/`ca`/`in` → canonical English landing, `de` → de-DE, `pl` → pl-PL, `af` → af-ZA; `in` вмикає India-атмосферу й явний Hindi toggle, старий `en-IN` лишається legacy stub без маршрутизації. Free план дає Universal SSL. **Міграція виконана (2026-09-09): Pages прийняв трафік — https://biosunlocktool.com/ це основний продакшн.** NAS **http://nlmhelp.keenetic.link:18080/** (bind-mount, редагування = миттєвий ефект) — тестовий поверх: уся перевірка змін перед деплоєм робиться там; у прод розкочується лише перевірене (синк канону → preflight → push → зелений Action → smoke test). Станом на 2026-09-11 останні Actions падають із Cloudflare API error `10000` через CI secret; локальний Wrangler deploy є тимчасовим fallback. Деталі — `README.md` (Production) та `docs/architecture-decisions.md`. Правила роботи агента — `AGENTS.md`.
...
- India market: канонічний короткий host `in.biosunlocktool.com` використовує той самий English canonical landing, але host-aware `data-market="india"` вмикає окрему абстрактну атмосферу з indigo/navy, saffron і dark green; Hindi вмикається явним client-side toggle; старий `en-IN` лишається legacy stub, а `india` — лише compatibility marker у client-side detection; US apex та невідомі host-и лишаються без цього override;
- Poland market: `pl.biosunlocktool.com/` внутрішньо обслуговує локальну повну копію `locales/pl-PL/` без зміни адреси, яка зберігає 8FC8-структуру та купівельний шлях, але перекладає основний UI польською і вмикає радикально іншу host-aware атмосферу `data-market="poland"` — графіт, теплий паперовий відтінок, кармінне світло й тонку діагональну фактуру;
- Germany market: `de.biosunlocktool.com/` внутрішньо обслуговує локальну повну копію `locales/de-DE/` без зміни адреси, яка перекладає основний UI німецькою і вмикає `data-market="germany"` — steel-blue інженерне поле, amber-сигнал і креслярську сітку з контрольованим червоним діагональним маркером;
- Africa market: `af.biosunlocktool.com/` внутрішньо обслуговує локальну повну копію `locales/af-ZA/` без зміни адреси, з англійським UI для міжрегіонального використання та `data-market="africa"` — глибокий indigo, теракотове світло, охряний акцент і стриманий геометричний textile-ритм; це оборотний дизайн-експеримент, а не спроба представляти весь континент однією культурою;
...
- sync status: CF root і `locales/en-US/` уже мають `main.css?v=20260910y` та `india-language.js?v=20260910k`, але canonical `versions/en-US-landing/` ще має `main.css?v=20260910x` без India language script; перед наступним релізом потрібне свідоме вирішення цієї byte-identity розбіжності.
...
**Ризики:**
- **Порядок тестування не порушувати** — у прод має потрапляти лише перевірене на NAS: зміна → перевірка на 18080 → синк у CF-репо → preflight → push → зелений Action → перевірка апекса. Якщо Action заблокований Cloudflare `10000`, це треба зафіксувати як CI blocker, а не називати реліз зеленим.
...
**План:**
1. ~~Міграція на Cloudflare Pages~~ — **виконано 2026-09-09**: biosunlocktool.com віддає канон (апекс/www), NAS лишається тестовим поверхом. На 2026-09-11 останній Action має Cloudflare API error `10000` через CI secret; локальний Wrangler deploy є тимчасовим fallback до виправлення secret.
...
- реліз у прод: синк канону в `vaoferi/biosunlocktool` (корінь + `locales/en-US/`, byte-identity) → `npm run build`/`git diff --check` → push → дочекатись зеленого Action (`gh run list -R vaoferi/biosunlocktool`) або зафіксувати CI blocker → `curl https://biosunlocktool.com/` + browser smoke маркери актуальної зміни;
...
- India smoke: у браузері `in.biosunlocktool.com` має `data-market="india"`, saffron `--accent`, `main.css?v=20260910y`, `india-language.js?v=20260910k`, робоче English ↔ Hindi перемикання, чисту консоль і canonical `https://biosunlocktool.com/`; apex має `data-market="us"`.
- Poland smoke: у браузері `pl.biosunlocktool.com/` без redirect має `lang="pl-PL"`, `data-market="poland"`, кармінний `--accent`, CSS `main.css?v=20260910y`, польські hero/products/steps і чисту консоль; legacy `/locales/pl-PL/` має вести на clean root; на staging `pl.nlmhelp.keenetic.link:18080` перевіряється саме host-aware фон.
- Germany smoke: у браузері `de.biosunlocktool.com/` без redirect має `lang="de-DE"`, `data-market="germany"`, amber `--accent`, CSS `main.css?v=20260910y`, німецькі hero/products/steps і чисту консоль; legacy `/locales/de-DE/` має вести на clean root; на staging `de.nlmhelp.keenetic.link:18080` перевіряється саме host-aware фон.
- Africa smoke: у браузері `af.biosunlocktool.com/` без redirect має `lang="en-ZA"`, `data-market="africa"`, ochre `--accent`, CSS `main.css?v=20260910y`, Africa hero/trust copy і чисту консоль; legacy `/locales/af-ZA/` має вести на clean root; на staging `af.nlmhelp.keenetic.link:18080` перевіряється саме indigo/terracotta host-aware фон.
...
- [x] Poland market experiment опубліковано в `locales/pl-PL/` з окремою атмосферою та польським UI; production smoke на `pl.biosunlocktool.com/` пройдено для clean URL, мови, акценту, canonical і overflow.
- [x] Germany market experiment опубліковано в `locales/de-DE/` з окремою атмосферою та німецьким UI; production smoke на `de.biosunlocktool.com/` пройдено для clean URL, мови, акценту, canonical і overflow.
- [x] Africa market experiment опубліковано в `locales/af-ZA/` з окремою атмосферою та англійським UI; production smoke на `af.biosunlocktool.com/` пройдено для clean URL, мови, акценту, CSS stamp, canonical і overflow.
