...
The staging landing reads the hostname client-side; `in` gets the India theme, `pl` gets the Poland atmosphere, and `de` gets the Germany atmosphere. The staging bind-mount still serves the canonical English copy; localized Polish/German copies are routed by Cloudflare Pages to `/locales/pl-PL/` and `/locales/de-DE/`.
