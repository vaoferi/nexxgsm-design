...
void main() {
  ...
  // Яскравість: 0.36 → 0.25 (приблизно -30% за останнім візуальним фідбеком).
  // Це ГОЛОВНА ручка яскравості візерунка; темна база (0.04) і градієнт низу нижче — окремі речі.
  // Мобільний фон зберігає кольоровий вау-ефект, але працює приблизно на 30%
  // темніше, щоб не конкурувати з текстом і картками.
  col *= intensity * 0.25;

  float highlight = pow(clamp((nA * 1.1 + ridge * 0.75) - 1.1, 0.0, 1.0), 2.2);
  col = mix(col, vec3(1.0, 0.96, 0.92), highlight * 0.13);
