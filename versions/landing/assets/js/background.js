...
// Adapted shader: darker blues/teals for better text contrast
const FRAG = `
...
void main() {
  ...
  // Darker palette: deep blues, teals, navy for contrast
  float base = nA * 0.82 + ridge * 0.18;
  vec3 col = vec3(
    0.05 + 0.35 * (0.5 + 0.5 * cos(6.28318 * (base + 0.02 + t * 0.07))),
    0.08 + 0.45 * (0.5 + 0.5 * cos(6.28318 * (base + 0.37 + t * 0.06))),
    0.15 + 0.55 * (0.5 + 0.5 * cos(6.28318 * (base + 0.72 + t * 0.065)))
  );
  col *= intensity;

  float highlight = pow(clamp((nA * 1.1 + ridge * 0.75) - 1.1, 0.0, 1.0), 2.2);
  col = mix(col, vec3(0.1, 0.4, 0.6), highlight * vec3(0.22, 0.16, 0.1));
...
const floorKeyboardMat = new THREE.MeshStandardMaterial({ color: 0x0a0e14, roughness: 0.92, metalness: 0.04 });
...
const hemiLight = new THREE.HemisphereLight(0xaaccff, 0x060608, 0.06);
